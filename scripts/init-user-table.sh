...
MYSQL_DATABASE="sewage_db"

# 检查MySQL是否运行
if ! docker ps | grep mysql57 > /dev/null; then
...
# 执行SQL脚本
docker exec -i mysql57 mysql -h$MYSQL_HOST -P$MYSQL_PORT -u$MYSQL_USER -p$MYSQL_PASSWORD $MYSQL_DATABASE < /dev/stdin << 'EOF'
-- 创建用户表
...
-- 插入测试用户账号（密码：user123）
INSERT INTO `sys_user` (`username`, `password`, `real_name`, `phone`, `email`, `department`, `role`, `status`)
VALUES ('user', '$2a$10$K8FpFzjJqj9JnK7m2qEJzOTUkq9KqT5kMkq7KqKqKqKqKqKqKqKqKqKq', '测试用户', '13800138002', 'user@sewage.com', '监控部', 'OPERATOR', 1)
ON DUPLICATE KEY UPDATE `password` = VALUES(`password`);
EOF
